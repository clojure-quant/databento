(ns indicator.decline
   (:require
    ...
    [indicator.prices :refer [add-future-prices]]
    [data.assets :refer [add-category]]))
...
(defn add-trailing-decline [asset-ds n]
  (let [{:keys [high low close]} asset-ds
        ...
        trailing-high-idx (-> (dfn/- idx (:trailing-ago result))
                              (dfn/max 0)
                              (dtype/clone))
        date-instant (into [] (map string-date-to-instant (:date asset-ds)))]
    ...
    (-> asset-ds
        (tc/add-columns (assoc result :prct-decline prct-decline
                        ...
                        :na)
        (tc/add-column :trailing-high-date (lookup-date date-instant trailing-high-idx)))))
